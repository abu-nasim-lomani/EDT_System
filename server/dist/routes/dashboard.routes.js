...
exports.dashboardRouter.use(auth_middleware_1.protectRoute);
// ── GET /api/dashboard/stats ──
// SM-level overview: all project/task/event counts
exports.dashboardRouter.get("/stats", (0, auth_middleware_1.requireRole)("SENIOR_MANAGEMENT"), async (_req, res) => {
    const [totalProjects, activeProjects, pendingProjects, stuckProjects, completedProjects, totalTasks, completedTasks, pendingTasks, stuckTasks, upcomingEvents, conflictCount,] = await Promise.all([
        prisma_1.prisma.project.count(),
        prisma_1.prisma.project.count({ where: { status: "IN_PROGRESS" } }),
        prisma_1.prisma.project.count({ where: { status: "PENDING" } }),
        prisma_1.prisma.project.count({ where: { status: "STUCK" } }),
        prisma_1.prisma.project.count({ where: { status: "COMPLETED" } }),
        ...
        prisma_1.prisma.task.count({ where: { status: "STUCK" } }),
        prisma_1.prisma.event.count({ where: { status: "SCHEDULED", startDate: { gte: new Date() } } }),
...
exports.dashboardRouter.get("/my-todo", async (req, res) => {
...
exports.dashboardRouter.get("/reports", (0, auth_middleware_1.requireRole)("SENIOR_MANAGEMENT"), async (req, res) => {
    ...
    const projectGroupBy = await prisma_1.prisma.project.groupBy({
        by: ["status"],
        _count: { id: true }
    });
    ...
    const eventsByType = await prisma_1.prisma.event.findMany({
    ...
    const usersWithTasks = await prisma_1.prisma.user.findMany({
    ...
    const conflictCount = await prisma_1.prisma.event.count({
        where: {
...
exports.dashboardRouter.get("/executive", (0, auth_middleware_1.requireRole)("SENIOR_MANAGEMENT"), async (req, res) => {
    try {
        const now = new Date();
        ...
        const [allProjects, allTasks, allEvents, recentDecisions] = await Promise.all([
            ...
            prisma_1.prisma.decision.findMany({
                take: 8,
...
exports.dashboardRouter.get("/pm", async (req, res) => {
    const userId = req.user.userId;
    ...
    try {
        ...
        const myTasks = await prisma_1.prisma.task.findMany({
        ...
        // ── 3. My Events ──
        const myEvents = await prisma_1.prisma.event.findMany({
            where: {
                OR: [
                    { creatorId: userId },
